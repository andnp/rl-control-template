...
from RlEvaluation.utils.pandas import split_over_columns, split_over_column
...
if __name__ == "__main__":
    ...
    for (env, alg), env_df in split_over_columns(df, cols=['environment', 'algorithm']):
        f, ax = plt.subplots()

        print(env, alg)
        for buffer_type, sub_df in split_over_column(env_df, col='buffer_type'):
            ...
            ax.plot(xs[0], res.sample_stat, label=buffer_type, color=COLORS[buffer_type], linewidth=0.5)
            ax.fill_between(xs[0], res.ci[0], res.ci[1], color=COLORS[buffer_type], alpha=0.2)
